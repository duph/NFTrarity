...
async function countRarity(raribleSdk : IRaribleSdk, collectionId : string, oFilePath : string){
    ...
    for (let i = 0; i < metadata.length; i = i + 1) {
        ...
        for (let j = 0; j < current.length; j = j + 1) {
            current[j].percentage = statsCollection[current[j].trait_type][current[j].value]/totalAmount ;
        }
        ...
        for (let j = 0; j < current.length; j = j + 1) {            // COUNTING RARITY FUNCTION MAIN THING
            totalRarity = totalRarity + Math.exp(10*(1-current[j].percentage));     // EXPONENTIAL RARITY FUNCTION
        }
        totalRarity = totalRarity / current.length - 1;
    ...
    let txtarr = [];
    for (let j = 0; j < arrRarityNFT.length; j++) {
        txtarr.push([j,arrRarityNFT[j].token_id,arrRarityNFT[j].rarity]);
    }
    const fs = require("fs");
    fs.writeFileSync(oFilePath, txtarr);
